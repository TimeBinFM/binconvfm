...
class BinConvForecaster(BaseForecaster):
    def __init__(
        ...
        self.scaler_type = self.scaler_type
    ...
    def _create_model(self):
        ...
        self.model = BinConvModule(
            ...
            lr = self.lr,
            is_prob_forecast = self.is_prob_forecast,
            context_length = self.context_length,
            num_bins = self.num_bins,
            min_bin_value = self.min_bin_value,
            max_bin_value = self.max_bin_value,
            kernel_size_across_bins_2d = self.kernel_size_across_bins_2d,
            kernel_size_across_bins_1d = self.kernel_size_across_bins_1d,
            num_filters_2d = self.num_filters_2d,
            num_filters_1d = self.num_filters_1d,
            num_1d_layers = self.num_1d_layers,
            num_blocks = self.num_blocks,
            kernel_size_ffn = self.kernel_size_ffn,
...
class BinConvModule(BaseLightningModule):
    def __init__(self, horizon, n_samples, quantiles, lr,
        ...
        self.model = BinConv(context_length=context_length,
                             is_prob_forecast=is_prob_forecast,
                             num_bins=num_bins,
    ...
    def loss(self, batch, batch_idx):
        ...
        logits_seq = self.model(input_seq)
        loss = F.binary_cross_entropy_with_logits(logits_seq, target_seq)
