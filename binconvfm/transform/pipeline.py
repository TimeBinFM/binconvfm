...
class Pipeline:
    ...
    def __init__(self, steps: List[Tuple[str, BaseTransform]]):
        ...
        for i, step in enumerate(steps):
            ...
            if not isinstance(transformer, BaseTransform):
                raise ValueError(f"Step {i} transformer must be a BaseTransform, "
    ...
    def fit(self, data: torch.Tensor) -> List[Dict[str, torch.Tensor]]:
        ...
        for i, (name, transformer) in enumerate(self.steps):
            ...
            current_data, _ = transformer.transform(current_data, step_params)
        
        return all_params
    
    def transform(self, data: torch.Tensor, params: List[Dict[str, torch.Tensor]] = None) -> Tuple[torch.Tensor, List[Dict[str, torch.Tensor]]]:
        """
        ...
        if params is not None and len(params) != len(self.steps):
            raise ValueError(f"Number of parameter sets ({len(params)}) must match "
        ...
        for i, (name, transformer) in enumerate(self.steps):
            ...
            current_data, used_params = transformer.transform(current_data, step_params)
            result_params.append(used_params)
        
        return current_data, result_params
    
    def inverse_transform(self, data: torch.Tensor, params: List[Dict[str, torch.Tensor]]) -> Tuple[torch.Tensor, List[Dict[str, torch.Tensor]]]:
        """
        ...
        # Apply inverse transformations in reverse order
        for i, ((name, transformer), step_params) in enumerate(zip(reversed(self.steps), reversed(params))):
            step_num = len(self.steps) - i
            logger.debug(f'Applying inverse transformation step {step_num}/{len(self.steps)}: {name}')
            current_data, used_params = transformer.inverse_transform(current_data, step_params)
            result_params.insert(0, used_params)  # Insert at beginning to maintain original order
        
        return current_data, result_params
    
    def fit_transform(self, data: torch.Tensor) -> Tuple[torch.Tensor, List[Dict[str, torch.Tensor]]]:
        """
        ...
        transformed_data, _ = self.transform(data, params)
        ...
        for name, transformer in self.steps:
            ...
            if hasattr(transformer, 'get_params'):
                ...
                params[name] = {
                    ...
                    'inverse_input_dims': getattr(transformer, 'inverse_input_dims', None)
                }
